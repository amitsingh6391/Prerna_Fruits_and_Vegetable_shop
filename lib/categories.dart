...
List category=[{"Child": "wait",
  "ID": 1, "IsChild": false, "Name": "Fruits",
...
class _categoriesState extends State<categories> {
  ...
  Future fetcategory() async {
    ...
    http.Response response = await http.get(
        "http://pfv.wonsoft.co.in/API/Post.asmx/GetCat");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text("Shop By Category"),backgroundColor: x,
          ),
          ...
          return  Card(
              child:GestureDetector(onTap: (){
                if(c==null){
                 Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => Products(category_id:category[index]["ID"])));
              }
                else
                  {print(c);
                  Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => subcategories(Object: c,)));
                  }
                },
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container( height:50,
                          child:Image(image:NetworkImage(category[index]["Img"],
                          ),
                              fit:BoxFit.fill
                          )),
                      Container(margin: EdgeInsets.only(top: 10),alignment: Alignment.center,
                          child: Text(category[index]["Name"],style: TextStyle(color: Colors.black),))
                    ],
                  ),
                ),)
          );
